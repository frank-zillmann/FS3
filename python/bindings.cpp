...
NB_MODULE(fs3, m) {
    ...
    nb::enum_<Phase>(m, "Phase").value("Liquid", Phase::Liquid).value("SolidOrSlurry", Phase::SolidOrSlurry);
    ...
    nb::class_<Component>(m, "Component")
        ...
        .def(
            "__init__",
            [](Component& self, const std::string& name, nb::kwargs kwargs) {
                ...
                if (kwargs.contains("charge")) self.setCharge(nb::cast<int>(kwargs["charge"]));
                if (kwargs.contains("type")) self.setType(nb::cast<Type>(kwargs["type"]));
                if (kwargs.contains("radius_m")) self.setRadius(nb::cast<double>(kwargs["radius_m"]));
                ...
                if (kwargs.contains("truesdell_jones_alpha_meter") &&
                    kwargs.contains("truesdell_jones_beta_cubicmeter_per_mol"))
                    self.setTruesdellJonesParameters(nb::cast<double>(kwargs["truesdell_jones_alpha_meter"]),
                                                     nb::cast<double>(
                                                         kwargs["truesdell_jones_beta_cubicmeter_per_mol"]));
                else if (kwargs.contains("truesdell_jones_alpha_meter") ||
                         kwargs.contains("truesdell_jones_beta_cubicmeter_per_mol"))
                    throw std::invalid_argument(
                        "Both truesdell_jones_alpha_meter and truesdell_jones_beta_cubicmeter_per_mol must be provided "
                        "together!");
            },
            "name"_a, "kwargs"_a)
    ...
    nb::class_<ReactionSystem>(m, "ReactionSystem")
        .def(nb::init<const ComponentSystem&, const ActivityModelBase&>(), "component_system"_a, "activity_model"_a,
    ...
    m.def("parse_mass_action_law", &parse_massActionLaw, "component_system"_a, "reaction_str"_a,
          "Parse a reaction string like 'A + B <-> C + D' and return (reactant_indices, product_indices)");

    m.def("mass_action_law", &massActionLaw, "component_system"_a, "reaction_str"_a, "k_forward"_a, "k_backward"_a,
          "error_component_idx"_a = -1, "Create a mass action law reaction from a string like 'H2O <-> H+ + OH-'");

    m.def("mass_action_law_damped", &massActionLaw_damped, "component_system"_a, "reaction_str"_a, "k_forward"_a,
          "k_backward"_a, "tau_reaction"_a, "error_component_idx"_a = -1,
    ...
    nb::class_<ArrayMapper>(m, "ArrayMapper")
        ...
        .def("__repr__", [](const ArrayMapper& m) {
            return "<ArrayMapper rows=" + std::to_string(m.rows) + " cols=" + std::to_string(m.cols) +
    ...
    nb::class_<Pipe, UnitOperationBase>(m, "Pipe")
        .def(nb::init<const ReactionSystem&, sunindextype, realtype, realtype, std::function<realtype(realtype)>, realtype>(),
    ...
    nb::class_<RS_MagneticCaptureProcessChamber, UnitOperationBase>(m, "RS_MagneticCaptureProcessChamber")
        .def(nb::init<const ReactionSystem&, sunindextype, realtype, realtype, realtype, sunindextype, realtype, realtype,
                      realtype, realtype, realtype, std::function<realtype(realtype)>, std::function<realtype(realtype)>,
                      std::function<realtype(realtype)>, std::function<realtype(realtype)>>(),
             "reaction_system"_a, "n_cells_per_phase"_a, "cross_section_area"_a, "length"_a, "empty_porosity"_a,
             "n_disks"_a, "disk_height"_a, "alpha_fluid_particle_volume_ratio"_a, "deposition_rate"_a, "MNP_capacity"_a,
             "magnetic_field_strength"_a, "a_eff_function"_a, "capture_function"_a, "flow_rate_function"_a,
    ...
    nb::class_<Process>(m, "Process")
        .def(nb::init<const ComponentSystem&, const std::vector<std::shared_ptr<UnitOperationBase>>&>(),
             "component_system"_a, "unit_operations"_a, nb::keep_alive<1, 2>())
        .def("add_connection", &Process::addConnection, "from_mapper"_a, "to_mapper"_a, "flow_rate_function"_a)
    ...
    nb::class_<TimeSeriesObserver>(m, "TimeSeriesObserver")
        .def(nb::init<realtype, realtype, std::size_t, const ArrayMapper&, bool>(), "t_start"_a, "t_end"_a,
             "n_snapshots"_a, "mapper"_a, "compute_errors"_a = false)
        .def(nb::init<realtype, realtype, std::size_t, const ArrayMapper&, Solver&, bool, bool>(), "t_start"_a,
             "t_end"_a, "n_snapshots"_a, "mapper"_a, "solver"_a, "add_to_solver"_a = true, "compute_errors"_a = false,
             nb::keep_alive<1, 5>())
        ...
        .def("__repr__", [](const TimeSeriesObserver&) { return "<TimeSeriesObserver>"; });

    // ==================== Solver ====================

    nb::class_<Solver>(m, "Solver")
        .def(nb::init<const Process&, SolverType>(), "process"_a, "solver_type"_a = SolverType::BDF, nb::keep_alive<1, 2>())
        .def("solve", &Solver::solve, "t_stop"_a = std::numeric_limits<realtype>::infinity(),
        ...
        .def("__repr__", [](const Solver& s) {
            return "<Solver t=" + std::to_string(s.getT()) + " y_size=" + std::to_string(s.getYSize()) + ">";
        });
    ...
    m.def("convert_to_pH", &convert_to_pH, "observer"_a, "cell_index"_a, "cell_volume"_a, "component_system"_a,
          "activity_model"_a, "Convert TimeSeriesObserver data to pH values for a specific cell");
}
